...
# Usage
# LLsub ./submit_checkpoint_eval.sh -s 20 -g volta:1 # TODO: potentially just use cpu

# Initialize and Load Modules
echo "[submit_checkpoint_eval.sh] Loading modules and virtual environment"
source /etc/profile
...
echo "[submit_checkpoint_eval.sh] Activating virtual environment"
...
echo "[submit_checkpoint_eval.sh] Setting wandb to offline"
wandb offline
...
echo "[submit_checkpoint_eval.sh] Running eval code..."
echo "[submit_checkpoint_eval.sh] Date: $DATE"
echo "[submit_checkpoint_eval.sh] Time: $TIME"

# Record start time
START_TIME=$(date +%s)

# Create temporary config file
echo "[submit_checkpoint_eval.sh] Creating temporary config file"
...
echo "[submit_checkpoint_eval.sh] Running eval command..."
...
echo "[submit_checkpoint_eval.sh] Job completed in ${HOURS}h ${MINUTES}m ${SECONDS}s"
